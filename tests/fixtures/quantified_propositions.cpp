...
law order_is_free(unsigned x)
    ensures(forall (unsigned a, unsigned b) { Eq<unsigned>(a + b, b + a) });
...
law shadowed(unsigned x)
    ensures(forall (unsigned x) { Eq<unsigned>(identity(x), x) });
...
law nested(unsigned x)
    ensures(forall (unsigned a) { forall (unsigned b) { a + b == b + a } });
...
law both_sides(unsigned x)
    expects(x == 0u)
    ensures(add_one(x) == 1u && identity(x) == 0u);
...
law from_one_side(unsigned x, unsigned y)
    expects(x == 1u && y == 2u)
    ensures(identity(x) == 1u);

law conjunction_chain(unsigned x)
    expects(x == 0u)
    ensures(x == 0u && add_one(x) == 1u && identity(x) == 0u);
...
law tighter_than_implication(unsigned x)
    ensures(x == 0u && x != 1u -> add_one(x) == 1u);

// Under a binder, and of a quantified body.
law everywhere_both(unsigned x)
    ensures(forall (unsigned y) { identity(y) == y && add_one(y) == y + 1u });
