...
law from_the_left(unsigned x) ensures(x == x || x == 1u);
law from_the_right(unsigned x) ensures(x == 1u || x == x);
proof from_the_left_holds(unsigned x) proves(from_the_left(x)) { refl; }

// Explicit equalities compose through `||` as they do through the other
// connectives.
proof formal_side(unsigned x) proves(Eq<unsigned>(identity(x), x) || Eq<unsigned>(x, 1u)) { refl; }

// A disjunctive premise is used by cases: the conclusion has to follow from each
// side on its own.
law bounded_by_cases(unsigned x) expects(x == 0u || x == 1u) ensures(x <= 1u);
law three_cases(unsigned x) expects(x == 0u || x == 1u || x == 2u) ensures(x <= 2u);

// The same premise, with the conclusion reached by rewriting inside each case.
law identity_by_cases(unsigned x) expects(x == 0u || x == 0u) ensures(identity(x) == 0u);

// `&&` binds tighter than `||`, which binds tighter than `->` (GRAMMAR.md 33).
// The looser readings of both are false at some value.
law conjunction_binds_tighter(unsigned x) ensures(x == 0u && x != 0u || x == x);
law implication_is_looser(unsigned x) ensures(x == 0u || x == 1u -> x <= 1u);

// Under a binder, and on either side of an implication.
law everywhere(unsigned x) ensures(forall (unsigned y) { y == y || y == 1u });
law guarded(unsigned x) ensures(x == 0u -> x == 0u || x == 1u);
