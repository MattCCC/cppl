...
proof formal_pair(unsigned x) proves(Eq<unsigned>(identity(x), x) && Eq<unsigned>(x, x)) { refl; }
proof reuse_pair(unsigned x) proves(Eq<unsigned>(identity(x), x) && Eq<unsigned>(x, x)) { exact formal_pair(x); }
proof applied_pair(unsigned x) proves(Eq<unsigned>(identity(x), x) && Eq<unsigned>(x, x)) { apply formal_pair(x); }

// One side written formally and the other as an ordinary C++ predicate.
proof mixed_pair(unsigned x) proves(Eq<unsigned>(identity(x), x) && x == x) { refl; }

proof quantified_pair(unsigned x)
    proves((forall (unsigned y) { Eq<unsigned>(y, y) }) && Eq<unsigned>(x, x)) { refl; }
...
law equivalent(unsigned x) ensures(Eq<unsigned>(x, 0u) <-> Eq<unsigned>(x, 0u));
law resolved_equivalence(unsigned x) ensures(x == 0u <-> identity(x) == 0u);
law precedence(unsigned x) ensures(x == 0u -> x == 0u <-> x == x);

// Provable only because `&&` binds tighter than `<->`: the looser reading
// `x == 0u && (x == x <-> x == 0u)` is false at every other value.
law conjunction_binds_tighter(unsigned x) ensures(x == 0u && x == x <-> x == 0u);

law quantified_equivalence(unsigned x)
    ensures(forall (unsigned y) { Eq<unsigned>(y, 0u) <-> Eq<unsigned>(y, 0u) });
proof reflexive_equivalence(unsigned x) proves(Eq<unsigned>(x, x) <-> Eq<unsigned>(identity(x), x)) { refl; }
proof reuse_equivalence(unsigned x) proves(Eq<unsigned>(x, x) <-> Eq<unsigned>(identity(x), x)) {
