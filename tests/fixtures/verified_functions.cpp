...
pure unsigned input(unsigned x) { return x; }
...
namespace nested {
verified pure int identity(int x) ensures(result == x) { return x; }
verified unsigned identity(unsigned x) ensures(result == x) { return x; }
}

law signed_identity(int x) ensures(nested::identity(x) == x);
law constant_equality(unsigned x) expects(0u == x) ensures(0u == 0u + 0u);

int main() {
    unsigned result = identity(41u);
    return result != 41u || inc(41u) != 42u || inc(~0u) != 0u ||
           zero_if_zero(0u) != 0u || one_if_zero(0u) != 1u ||
           first(2u, 3u) != 2u || second(2u, 3u) != 3u ||
           equal_inputs(4u, 4u) != 5u || from_pure(6u) != 6u ||
           constant() != 7u || nested::identity(8) != 8 ||
           nested::identity(9u) != 9u || zero_if_zero(5u) != 5u ||
