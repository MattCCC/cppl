...
k::Term number(std::int64_t value) { return k::Term::literal(u32.integer_type(), value); }
k::Term variable(std::uint32_t index = 0) { return k::Term::variable(k::VarIndex{index}); }
...
k::ProofTerm clamp_proof() {
    ...
    return k::ProofTerm::conditional_elimination(u32, condition, variable(), number(10),
            bounded(variable()),
            k::ProofTerm::implication_introduction(k::predicate(condition, true),
                k::ProofTerm::hypothesis(k::HypothesisIndex{0})),
            k::ProofTerm::implication_introduction(k::predicate(condition, false),
                k::ProofTerm::reflexivity()));
}

k::Proposition clamp_goal() {
    return k::Proposition::for_all(u32, bounded(k::Term::primitive(k::PrimOp::Select,
        u32.integer_type(), {compare(k::PrimOp::LessEqual, variable(), number(10)), variable(), number(10)})));
}
...
CPPL_TEST(comparison_primitives_compute_exact_signed_and_unsigned_results) {
    ...
    for (std::size_t index = 0; index < 6; ++index) {
        for (const auto& type : {u32, i32}) {
            ...
            CPPL_CHECK(!k::check({}, k::predicate(condition, !results[index]),
                                 k::ProofTerm::reflexivity(), {}));
        }
    }
...
CPPL_TEST(negated_comparisons_preserve_equality_and_boolean_polarity) {
    ...
    CPPL_CHECK(k::check({}, k::Proposition::equality(u32, selected, number(2)),
                        k::ProofTerm::reflexivity(), {}).has_value());
}

CPPL_TEST(path_rule_does_not_introduce_order_weakening) {
    const auto strict = compare(k::PrimOp::Less, variable(), number(10));
    const auto goal = k::Proposition::for_all(u32, k::Proposition::implication(
        k::predicate(strict, true), bounded(variable())));
    const auto proof = k::ProofTerm::forall_introduction(u32, k::ProofTerm::implication_introduction(
        k::predicate(strict, true), k::ProofTerm::hypothesis(k::HypothesisIndex{0})));
