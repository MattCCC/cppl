...
CPPL_TEST(every_quantifier_binder_is_validated_even_when_unused) {
    for (const std::uint16_t width : {std::uint16_t{0}, std::uint16_t{65},
        ...
        for (const bool used : {false, true}) {
            ...
            const auto body = used ? k::Proposition::equality(invalid, variable, variable)
                                   : identity();
            const auto goal = k::Proposition::for_all(invalid, body);
            const auto evidence = k::ProofTerm::forall_introduction(invalid,
    ...
    for (const auto width : {1u, 32u, 64u}) {
        const auto valid = k::Type::integer(static_cast<std::uint16_t>(width),
                                            k::Signedness::Unsigned);
        CPPL_CHECK(k::check({}, k::Proposition::for_all(valid, identity()),
            k::ProofTerm::forall_introduction(valid, k::ProofTerm::reflexivity()), {}).has_value());
    }
}
...
CPPL_TEST(unknown_signedness_never_becomes_a_core_type) {
    for (unsigned tag = 2; tag <= 255; ++tag) {
        ...
        CPPL_CHECK(!k::check({}, k::Proposition::equality(invalid, zero, zero),
                            k::ProofTerm::reflexivity(), {}).has_value());
        CPPL_CHECK(!k::check({}, k::Proposition::for_all(invalid, identity()),
            k::ProofTerm::forall_introduction(invalid, k::ProofTerm::reflexivity()), {}).has_value());
        k::Context context;
        k::Definition definition{k::DefId{0}, "invalid", {invalid}, u32,
                                  k::Term::literal(u32.integer_type(), 0)};
...
CPPL_TEST(a_free_variable_cannot_import_an_invalid_local_type) {
    for (const auto type : {k::Type::integer(0, k::Signedness::Unsigned),
                            k::Type::integer(32, static_cast<k::Signedness>(255))}) {
