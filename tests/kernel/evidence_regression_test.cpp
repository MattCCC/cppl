...
k::Term lit(std::int64_t n) { return k::Term::literal(u32.integer_type(), n); }
k::Term var(std::uint32_t n) { return k::Term::variable(k::VarIndex{n}); }
...
CPPL_TEST(implication_elimination_derives_its_conclusion_independently) {
    ...
    const auto proof = k::ProofTerm::implication_elimination(implication,
        k::ProofTerm::implication_introduction(truth, k::ProofTerm::reflexivity()),
...
CPPL_TEST(implication_elimination_checks_the_claimed_theorem_recursively) {
    ...
    const auto proof = k::ProofTerm::implication_elimination(claimed,
        k::ProofTerm::implication_introduction(truth, k::ProofTerm::reflexivity()),
        k::ProofTerm::reflexivity());
    CPPL_CHECK(!k::check({}, falsehood, proof, {}).has_value());
}
...
CPPL_TEST(transport_checks_equality_when_both_motive_instances_fit) {
    ...
    const auto proof = k::ProofTerm::equality_elimination(u32, lit(0), lit(1),
        eq(var(0), lit(1)), k::ProofTerm::reflexivity(), k::ProofTerm::reflexivity());
    CPPL_CHECK(!k::check({}, falsehood, proof, {}).has_value());
    const auto valid = k::ProofTerm::equality_elimination(u32, lit(1), lit(1),
        eq(var(0), lit(1)), k::ProofTerm::reflexivity(), k::ProofTerm::reflexivity());
...
CPPL_TEST(conditional_elimination_cannot_accept_a_goal_outside_its_motive) {
    ...
    const auto proof = k::ProofTerm::conditional_elimination(u32, condition, lit(0), lit(0),
        eq(var(0), lit(0)),
