...
k::Term lit(const k::IntType& type, std::int64_t value) { return k::Term::literal(type, value); }
...
k::Term add(const k::IntType& t, k::Term a, k::Term b) { return prim(k::PrimOp::AddWrap, t, {a, b}); }
k::Term sub(const k::IntType& t, k::Term a, k::Term b) { return prim(k::PrimOp::SubWrap, t, {a, b}); }
k::Term mul(const k::IntType& t, k::Term a, k::Term b) { return prim(k::PrimOp::MulWrap, t, {a, b}); }
...
bool definitional(const k::IntType& type, const k::Term& lhs, const k::Term& rhs,
                  std::uint32_t binders = 2, const k::Type* result = nullptr) {
...
std::int64_t as_signed(const k::IntType& type, std::uint64_t bits) {
    if (type.width >= 64) return static_cast<std::int64_t>(bits);
...
std::uint64_t evaluate(const k::Term& term, const std::vector<std::uint64_t>& environment) {
    ...
    for (const auto& argument : node.arguments) values.push_back(evaluate(argument, environment));
    const auto& t = node.type;
    switch (node.op) {
        case k::PrimOp::AddWrap: return (values[0] + values[1]) & mask(t);
        case k::PrimOp::SubWrap: return (values[0] - values[1]) & mask(t);
        case k::PrimOp::MulWrap: return (values[0] * values[1]) & mask(t);
        case k::PrimOp::Equal: return values[0] == values[1] ? 1u : 0u;
        case k::PrimOp::NotEqual: return values[0] != values[1] ? 1u : 0u;
        case k::PrimOp::Less: return before(t, values[0], values[1]) ? 1u : 0u;
        case k::PrimOp::LessEqual: return !before(t, values[1], values[0]) ? 1u : 0u;
        case k::PrimOp::Greater: return before(t, values[1], values[0]) ? 1u : 0u;
        case k::PrimOp::GreaterEqual: return !before(t, values[0], values[1]) ? 1u : 0u;
        case k::PrimOp::Not: return values[0] == 0 ? 1u : 0u;
        case k::PrimOp::Select: return values[0] != 0 ? values[1] : values[2];
    }
    return 0;
}
...
k::Term random_condition(Random& random, const k::IntType& type, std::uint32_t variables, int depth) {
    static const k::PrimOp comparisons[] = {k::PrimOp::Equal, k::PrimOp::NotEqual, k::PrimOp::Less,
                                            k::PrimOp::LessEqual, k::PrimOp::Greater,
                                            k::PrimOp::GreaterEqual};
    k::Term condition = prim(comparisons[random.below(6)], type,
                             {random_term(random, type, variables, depth),
                              random_term(random, type, variables, depth)});
    if (random.below(4) == 0) condition = prim(k::PrimOp::Not, k::kBoolean, {condition});
    return condition;
}
...
k::Term random_term(Random& random, const k::IntType& type, std::uint32_t variables, int depth) {
    ...
    switch (choice) {
        case 0: return var(static_cast<std::uint32_t>(random.below(variables)));
        case 1: return lit(type, k::wrap_into(type, static_cast<std::int64_t>(random.next())));
        case 2: return add(type, random_term(random, type, variables, depth - 1),
                           random_term(random, type, variables, depth - 1));
        case 3: return sub(type, random_term(random, type, variables, depth - 1),
                           random_term(random, type, variables, depth - 1));
        case 4: return mul(type, random_term(random, type, variables, depth - 1),
                           random_term(random, type, variables, depth - 1));
        case 5: return prim(k::PrimOp::Select, type,
                            {random_condition(random, type, variables, depth - 1),
                             random_term(random, type, variables, depth - 1),
                             random_term(random, type, variables, depth - 1)});
        case 6: return mul(type, lit(type, k::wrap_into(type, static_cast<std::int64_t>(random.below(7)) - 3)),
                           random_term(random, type, variables, depth - 1));
        case 7: return add(type, random_term(random, type, variables, depth - 1), lit(type, 1));
        default: return sub(type, random_term(random, type, variables, depth - 1),
...
void each_assignment(const k::IntType& type, std::uint32_t variables,
    ...
    for (std::uint32_t index = 0; index < variables; ++index) total *= size;
...
void check_normal_forms_agree(const k::IntType& type, std::uint64_t seed, int terms) {
    ...
    for (int index = 0; index < terms; ++index) {
        const k::Term term = index % 3 == 0 ? random_condition(random, type, 2, 3)
                                            : random_term(random, type, 2, 4);
...
CPPL_TEST(normal_forms_agree_at_64_bits_at_the_edges_of_the_type) {
    for (const auto& type : {k::IntType{64, k::Signedness::Unsigned}, k::IntType{64, k::Signedness::Signed}}) {
        ...
        const std::uint64_t edges[] = {0u, 1u, ~std::uint64_t{0}, std::uint64_t{1} << 63,
                                       (std::uint64_t{1} << 63) - 1u, 0x5555555555555555u};
...
CPPL_TEST(comparisons_are_decided_only_where_the_machine_type_decides_them) {
    ...
    CPPL_CHECK(definitional(kU32, prim(k::PrimOp::Equal, kU32, {add(kU32, x, lit(kU32, 1)), add(kU32, y, lit(kU32, 1))}),
                            prim(k::PrimOp::Equal, kU32, {x, y}), 2, &boolean));
    CPPL_CHECK(!definitional(kU32, prim(k::PrimOp::Less, kU32, {add(kU32, x, lit(kU32, 1)), add(kU32, y, lit(kU32, 1))}),
    ...
    CPPL_CHECK(definitional(kU32, prim(k::PrimOp::Greater, kU32, {x, y}), prim(k::PrimOp::Less, kU32, {y, x}), 2, &boolean));
...
CPPL_TEST(a_normalization_budget_rejects_an_exploding_product) {
    ...
    k::Term sum = var(0);
    for (std::uint32_t index = 1; index < 8; ++index) sum = add(kU32, sum, var(index));
    k::Term power = sum;
    for (int index = 1; index < 6; ++index) power = mul(kU32, power, sum);
...
CPPL_TEST(malformed_or_insufficient_certificates_are_refused) {
    ...
    CPPL_CHECK(!bounded(k::ArithmeticCertificate{k::DisjunctionCases{
        0, k::Box<k::ArithmeticCertificate>{farkas({{2, 1}, {3, 1}})},
        k::Box<k::ArithmeticCertificate>{farkas({{2, 1}, {3, 1}})}}}).has_value());
    CPPL_CHECK(!bounded(k::ArithmeticCertificate{k::IntegerSplit{
        {{7, 1}}, 0, k::Box<k::ArithmeticCertificate>{farkas({{2, 1}, {3, 1}})},
        k::Box<k::ArithmeticCertificate>{farkas({{2, 1}, {3, 1}})}}}).has_value());
}
...
CPPL_TEST(a_split_examines_both_sides_and_each_must_be_refuted) {
    ...
    const auto both = k::ArithmeticCertificate{k::IntegerSplit{
        {{0, 1}}, -10, k::Box<k::ArithmeticCertificate>{farkas({{3, 1}, {4, 1}})},
        k::Box<k::ArithmeticCertificate>{farkas({{2, 1}, {4, 1}})}}};
    CPPL_CHECK(bounded(both).has_value());
    // A side left unrefuted fails the whole step.
    const auto one_sided = k::ArithmeticCertificate{k::IntegerSplit{
        {{0, 1}}, -10, k::Box<k::ArithmeticCertificate>{farkas({{3, 1}, {4, 1}})},
...
CPPL_TEST(arithmetic_facts_must_carry_checked_evidence) {
    ...
    unintroduced.push_back(k::ArithmeticFact{premise, k::Box<k::ProofTerm>{k::ProofTerm::hypothesis(k::HypothesisIndex{0})}});
