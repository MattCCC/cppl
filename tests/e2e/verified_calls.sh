...
CLANG="$4"
mkdir -p "$WORK"
run=$(mktemp -d "$WORK/verified-calls.XXXXXX")
for standard in c++17 c++20 c++23; do
    ...
    grep -Eq '^Call preconditions proven: +8$' "$run/report"
    grep -Eq '^Unresolved obligations: +0$' "$run/report"
    grep -Eq '^Trusted external axioms: +0$' "$run/report"
    "$run/program"
    grep -Fq 'return inc(inc(x));' "$run/runtime.cpp"
    grep -Fq 'return bump_one(bump_zero(x));' "$run/runtime.cpp"
    if grep -Eq '\b(verified|ensures|expects|assert)\b' "$run/runtime.cpp"; then
        echo 'contract syntax or runtime checks survived erasure' >&2
        exit 1
    fi
...
echo 'verified calls compose contracts while preserving native calls'
