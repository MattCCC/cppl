...
for standard in c++17 c++20 c++23; do
    ...
    grep -Eq '^Laws proven: +18$' "$run/$standard.log"
    grep -Eq '^  by a written proof: +8$' "$run/$standard.log"
    grep -Eq '^Proof declarations proven: +5$' "$run/$standard.log"
    grep -Eq '^Function contracts proven: +4$' "$run/$standard.log"
    grep -Eq '^Laws trusted: +0$' "$run/$standard.log"
    grep -Eq '^Unresolved obligations: +0$' "$run/$standard.log"
    test "$("$run/$standard")" = '40 1 3 2'
done
