...
Proposition reflexive_goal() {
    return Proposition::for_all(
        signed32(), Proposition::equality(signed32(), Term::variable(VarIndex{0}),
                                          Term::variable(VarIndex{0})));
}
...
CPPL_TEST(a_kernel_acceptance_for_this_goal_yields_proven) {
    ...
    const auto accepted = cppl::kernel::check(
        context, goal, ProofTerm::forall_introduction(signed32(), ProofTerm::reflexivity()),
        CoreLimits{});
    CPPL_CHECK(accepted.has_value());

    const cppl::obligations::Verdict verdict =
        cppl::obligations::Verdict::proven(*accepted, obligation_for(goal));
...
CPPL_TEST(an_acceptance_for_another_goal_does_not_prove_this_one) {
    const cppl::kernel::Context context;
    const auto accepted = cppl::kernel::check(
        context, reflexive_goal(),
        ProofTerm::forall_introduction(signed32(), ProofTerm::reflexivity()), CoreLimits{});
    CPPL_CHECK(accepted.has_value());

    // A different proposition entirely: 1 == 2.
    const Proposition other = Proposition::equality(signed32(), Term::literal(kSigned32, 1),
                                                    Term::literal(kSigned32, 2));

    const cppl::obligations::Verdict verdict =
        cppl::obligations::Verdict::proven(*accepted, obligation_for(other));
...
CPPL_TEST(an_unresolved_verdict_carries_its_reason) {
    const cppl::obligations::Verdict verdict =
        cppl::obligations::Verdict::unresolved("no evidence was produced");
