...
k::Proposition holds(k::PrimOp op, const k::IntType& type, k::Term a, k::Term b, bool value = true) {
    return k::Proposition::equality(k::Type{k::kBoolean},
                                    k::Term::primitive(op, type, {std::move(a), std::move(b)}),
...
k::Proposition closed(const k::IntType& type, std::size_t binders,
                      std::vector<k::Proposition> premises, k::Proposition conclusion) {
...
CPPL_TEST(a_loop_step_preserves_its_bound) {
    ...
    const auto n = var(1);
    const auto i = var(0);
    CPPL_CHECK(proven(closed(kU32, 2,
                             {holds(k::PrimOp::LessEqual, kU32, i, n), holds(k::PrimOp::Less, kU32, i, n)},
...
CPPL_TEST(a_loop_exit_pins_its_counter) {
    ...
    const auto i = var(0);
    CPPL_CHECK(proven(closed(kU32, 2,
                             {holds(k::PrimOp::LessEqual, kU32, i, n),
...
CPPL_TEST(wrapping_is_respected_at_every_width) {
    ...
    CPPL_CHECK(!proven(closed(kU64, 1, {}, holds(k::PrimOp::Less, kU64, add(kU64, x, lit(kU64, 1)),
                                                  lit(kU64, 11)))));
}
...
CPPL_TEST(contradictory_premises_prove_any_arithmetic_goal_and_nothing_else_does) {
    ...
    const auto y = var(0);
    CPPL_CHECK(proven(closed(kU32, 2,
                             {holds(k::PrimOp::Less, kU32, x, y), holds(k::PrimOp::Less, kU32, y, x)},
                             equal(kU32, x, lit(kU32, 7)))));
    CPPL_CHECK(!proven(closed(kU32, 2,
                              {holds(k::PrimOp::LessEqual, kU32, x, y), holds(k::PrimOp::LessEqual, kU32, y, x)},
                              equal(kU32, x, lit(kU32, 7)))));
}
...
CPPL_TEST(evidence_for_one_goal_is_not_evidence_for_a_stronger_one) {
    const auto n = var(1);
    const auto i = var(0);
    const auto preserved = closed(kU32, 2,
                                  {holds(k::PrimOp::LessEqual, kU32, i, n), holds(k::PrimOp::Less, kU32, i, n)},
                                  holds(k::PrimOp::LessEqual, kU32, add(kU32, i, lit(kU32, 1)), n));
    const auto stronger = closed(kU32, 2,
                                 {holds(k::PrimOp::LessEqual, kU32, i, n), holds(k::PrimOp::Less, kU32, i, n)},
