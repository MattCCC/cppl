...
| By-value parameter members are callee storage | positive, negative, adversarial | covered — a member of a by-value aggregate parameter is written and read back, a nested member likewise, a sibling keeps its incoming value without becoming known, and a refined member owes its predicate on the way in; a reference parameter gets none of it, which is mutation-checked (`e2e/refinement_flow.sh` citing `STORAGE-011`) |
