...
| Qualifiers and overloads | positive, negative | covered — an overload on the implicit object's constness, `const&` and `&&` qualified member functions, a static member function; a `const` member function writing a `mutable` member is not relied on to leave it, and binding it `const` is mutation-checked; a volatile member function refused, which is mutation-checked (`fixtures/verified_methods.cpp`, `negative/verified_methods.sh` citing `CLASS-009`, `CLASS-012`, `CONTRACT-010`; `const-receiver-mutable-member`, `volatile-member-function-refused`) |
| Rvalue receivers | positive, negative | covered — an `&&` member function called by a verified caller through `std::move(m)` and `static_cast<Meter&&>(m)` on `m`'s own places; a stale claim after the call refused, and a call on a copy of the object refused as a temporary; reading `std::move(m)` as `m` is mutation-checked (`fixtures/verified_methods.cpp`, `e2e/verified_methods.sh`, `negative/verified_methods.sh` citing `CLASS-009`, `CLASS-011`; `rvalue-receiver-is-the-object`) |
| Pointer receivers | positive, negative, adversarial | covered — a mutating and a `const` call on what a pointer parameter designates, read back through `p->a`; a call without `readable(p)` and a mutating one without `writable(p)` refused, which is mutation-checked; a reference write that may be `p->v` takes away what was read before it (`fixtures/verified_methods.cpp`, `negative/verified_methods.sh` citing `CLASS-011`, `VERIFIED-038`; `pointer-receiver-capability`) |
| Refused receivers | negative | covered — a mutating call on an object a parameter designates by reference, a call on an element of an array of class type at a term, and a call on a temporary, each refused where the call is written (`negative/verified_methods.sh` citing `CLASS-011`, `CLASS-015`) |
| Member writes and refinements | positive, negative | covered — a write to one member keeps a sibling's fact, a refined member holds on entry, a refined return; a write that breaks a member's refinement refused, and dropping the member's refinement is mutation-checked (`fixtures/verified_methods.cpp`, `negative/verified_methods.sh` citing `CLASS-010`, `REFINEOBL-007`; `member-refinement-kept`) |
| Refinement validity by route | positive, negative, adversarial | covered — an in-range write through a reference that may be the member, an assignment after an unsafe block, and a call's effect through a `Small&` parameter each accepted with nothing owed at the return; an out-of-range aliasing write, a call's out-of-range effect, a return after an unsafe block, and an in-range aliasing write after an unsafe block each refused; charging the aliasing write, keeping validity only from a valid version, charging the return for an uncharged version, and charging a call's effect are each mutation-checked (`fixtures/verified_methods.cpp`, `negative/verified_methods.sh` citing `CLASS-010`, `REFINE-060`, `REFINE-061`, `REFINE-062`; `alias-write-charged`, `alias-write-validity-from-prior`, `return-charges-unestablished`, `call-effect-refinement-charged`) |
| Overlapping members | positive, negative | covered — scalar members kept apart beside an empty `[[no_unique_address]]` member and a bit-field; a reference member, a bit-field, a member of an anonymous union, a member of an anonymous struct and a volatile member refused where named (`fixtures/methods_scalar_disjoint.cpp`, `negative/verified_methods.sh` citing `CLASS-010`, `CLASS-015`) |
| Static members | positive, negative | covered — a `static pure` member function in a member's and a function's contract; a contract false of it refused (`fixtures/verified_methods.cpp`, `negative/verified_methods.sh` citing `CLASS-012`) |
...
| Aliasing | adversarial | covered — a reference parameter designating a member, a reference to the whole object, a `const` call on an object whose member a reference argument aliases, a call writing through a reference that may be a member, an unsafe block writing the object; tracking the object as storage no reference reaches, and filtering a call's writes by anything but the common alias model, are mutation-checked (`negative/verified_methods.sh` citing `CLASS-010`, `UNSAFE-005`; `receiver-caller-storage`, `call-effect-common-alias-model`) |
| Places a call cannot write | positive, adversarial | covered — a `const` call writing a reference to storage apart from the object keeps the object's facts, and the same call through the object's own member takes them away; a `const` call on the implicit object through a reference parameter that may be a member takes that member's fact away, and its twin reading first is accepted; a refined member such a call may reach takes a post-call version the callee's contract describes, `Small` included, so the caller's return owes nothing for it; keeping every place the callee only reads is mutation-checked (`fixtures/verified_methods.cpp`, `negative/verified_methods.sh` citing `CLASS-011`; `read-only-position-kept-apart`) |
| One post-state per storage | positive, regression | covered — a member passed as the reference argument too shares one post-call version, which the callee's postcondition about the member describes; a reference parameter's post-state stated after the calls in the returned value, whose earlier statement proved a false claim, is mutation-checked (`fixtures/verified_methods.cpp`, `negative/verified_storage.sh` citing `CLASS-011`, `VERIFIED-031`; `post-state-after-returned-call`) |
...
| Erasure | erasure | covered — identical output and assembly against a hand-erased twin in three standards, with size, alignment and a member offset asserted on both sides; each member function keeps the mangled name of its written signature and the program defines exactly its twin's symbols (`e2e/erasure_equivalence.sh`, `e2e/verified_methods.sh` citing `CLASS-013`, `ABI-002`) |
...
| Products | positive, negative | covered — a constant factor one past the bound at 32 and 64 bits; products of promoted 8- and 16-bit values accepted, of two `unsigned short` values refused unbounded and accepted with a bounded factor (`negative/signed_arithmetic.sh`, `fixtures/signed_arithmetic.cpp` `unsigned_short_doubled`) |
| Promotions and the common type | positive, negative | covered — two `unsigned char` values add as `int`, 510 not the wrapped 254, each side of the pair; `char16_t`, `char32_t` and `wchar_t` refused where named; bit-field reads refused, a 31-bit one promoting to `int` and a 32-bit one to `unsigned int`; refusing them is mutation-checked (`negative/signed_arithmetic.sh`, `fixtures/signed_arithmetic.cpp` `unsigned_char_sum` citing `ARITH-003`; `bit-field-read-refused`) |
| No self-justification | positive, negative, adversarial | covered — a result tested after the operation, a postcondition claimed of it and a modular identity (`(x + 1) - 1 == x`, in a body and in a specification) each leave the obligation unproven, and each twin with `expects (x < INT_MAX)` is accepted; supposing an obligation while proving it is mutation-checked (`negative/signed_arithmetic.sh` citing `ARITH-006`, `ARITH-009`; `definedness-not-self-supposed`) |
| Ring interpretation | adversarial | covered — for every pinned 4-bit signed pair the ring sum, difference, product by a constant and negation equal the exact result once the obligation is a premise, and their neighbours are not proven; `-1 + -1`, `MIN + 0` and `-MAX - 1` at 32 bits; `x + y > x`, `x - y < x`, `2 * x > x` and `-x > 0` proven only with the obligation, and no wrapped sum claimed larger or representable (`unit/definedness_arithmetic_test.cpp` citing `ARITH-006`, `ARITH-001`) |
| Each evaluation | positive, negative | covered — a template's `a + b` refused at `int` and accepted at `unsigned` and `unsigned long long`; a callee's obligation under its precondition, owed at each call site, the second call refused without its bound; every loop iteration under the invariant (`negative/signed_arithmetic.sh`, `fixtures/signed_arithmetic.cpp` `wrapped_sum`, `second_successor`, `thousands` citing `ARITH-009`) |
...
| Standard libraries | conformance | covered — the same claims against libc++ (macOS) and libstdc++ (`make ci-linux-gcc`), including a construction whose `std::allocator` argument libstdc++ defaults and libc++ takes in an overload; setting that argument aside is mutation-checked (`e2e/containers.sh` citing `STDMODEL-010`; `container-default-allocator`) |
| Machine arithmetic at the container | positive, negative | covered — a signed index is bounded as the size-type value C++ converts it to, and a division by a length owes a non-empty container; each without its guard is refused (`fixtures/containers.cpp`, `negative/containers.sh` citing `ARITH-008`, `ARITH-009`) |
| The parser shape | positive, negative | covered — characters of a span compared after promotion, a prefix a callee found appended to a caller's vector, run on a string; the same function claiming a strict bound is refused, and an element read directly in a condition is refused (`fixtures/containers.cpp` `skip_digits`, `collect_digits`; `negative/containers.sh` citing `STDMODEL-016`) |
