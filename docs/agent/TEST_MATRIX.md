...
| Symbolic element place identity | positive, negative, adversarial | covered — a subscript at a term is the place that term selects, so `a[i]` and `a[j]` transport no fact between them, a reassigned index names another element, and the same holds through a capability, where each subscript owes its own `readable`; one index term, including a compound one, still names one place (`negative/verified_storage.sh`, `e2e/refinement_flow.sh` citing `STORAGE-010`, `TCB-ALIAS-006`) |
